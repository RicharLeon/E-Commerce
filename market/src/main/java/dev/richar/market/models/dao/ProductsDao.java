...
public interface ProductsDao extends JpaRepository<Products, Integer>, JpaSpecificationExecutor<Products> {
    ...
    @Query("select new dev.richar.market.models.dto.ProductsConsultaDTO(p.id, p.name, p.idCategory, c.name, p.price, p.stock, p.status, p.photo, p.createdAt, p.updatedAt) from Products p " +
